...
flowchart TB
    ...
    subgraph 应用层
        B --> C[dedao_piracy_detector.py<br/>盗版检测主程序]
        B --> D[run_agent.py<br/>交互式操控程序]
    end
    
    subgraph Agent层
        C --> E[PiracyDetector<br/>盗版检测Agent]
        D --> F[PhoneAgent<br/>手机操控Agent]
        E --> F
        
        subgraph 双Agent架构
            G[Agent1: DeepSeek<br/>任务分解与提示词生成]
            H[Agent2: AutoGLM<br/>手机操控执行]
            G --> H
        end
        F --> G
        F --> H
    ...
    subgraph 核心模块
        F --> I[ModelClient<br/>模型客户端]
        F --> J[ActionHandler<br/>动作处理器]
        F --> K[ContentAnalyzer<br/>内容分析器]
    end
    
    subgraph ADB通信层
        J --> L[ADB模块]
        L --> M[screenshot.py<br/>截图]
        L --> N[device.py<br/>设备控制]
        L --> O[input.py<br/>文本输入]
        L --> P[connection.py<br/>连接管理]
    end
